...
| 上半身起跳、速度、置信度、一次起跳与落地重置 | 鼻尖与肩中心共同上移，肩宽归一化，头肩相对形状稳定；单人／同时／交替／倾斜跳合成测试 | 已实现并合成验证；无需下半身关节 |
| 区分真实抬手、侧倾、蹲起、踮脚、小跳 | 合成抬手／侧倾／蹲起回弹／小幅整体上移／点头／耸肩不触发 | 真人误触／漏检率未知；只观测上半身时，高幅踮脚与真跳若上半身轨迹相同则无法严格分离 |
...
| 显示 30 FPS、物理 60 Hz，五场景长帧 | 预热后每场景 30 秒：29.07／30.00／29.69／29.97／29.34 FPS，59.91–59.97 物理步／秒；最大帧间隔 86.9 ms | 已测量，持续稳定目标未完全证明 |
| 相机／每人姿态／显示率独立报告 | poseTelemetry／savePoseTelemetry；区域率与绑定玩家率分开，单人推理总次数单列 | 已实现计时；没有有效两人站位时不报告有效双人率 |
| 争取约 150 ms 端到端，并判断手感 | 延长采样的软件采集调用至 drawnow 均值 98.5–112.7 ms、各场景最大 168.1–230.3 ms | 曝光、内部缓存、屏幕呈现和真人动作起点未知；未达完整验收 |
| Code Analyzer 和现有逻辑回归 | sampling-checks、final-regression；新增路线输入适配后另跑 journey-adapter-regression | 以对应日志为准，不以测试名推断覆盖 |
| 窄平台、车尾和整局体感路线 | runPoseJourneyCheck：10／12 Hz 合成关节、83 ms 交付延迟、0.5 秒跳跃轨迹、原控制核心和实际物理；只提前规划起跳后上／下路均通过；10 Hz 时间边界差异已复现、修复并重跑 | 合成整路通过；真人／图像识别／手感仍未通过 |
| 可启动版本、Windows 命令、实际依赖、限制交付 | docs/pose-control-windows.md；实际窗口启动及切换已执行 | 实验版可启动；完整双人验收未完成 |

## 下一步边界

整路夹具仍是自动策略：它使用完整游戏状态提前规划，并非图像推理或真人决策。最初直接策略在北湖失败，整体提前预测在登录重试停滞；只对起跳提前规划后，12 Hz 上／下路为 59.68／72.05 秒。初轮 10 Hz 出现少数身体起跳未分类，已通过逐采样轨迹定位为 0.3 秒站稳边界的浮点舍入；仅补偿两个时间戳浮点刻度后，10 Hz 上／下路为 71.53／72.15 秒，全部 25／27 与 18／17 次身体起跳被分类并消费。旧失败和差异不删除，不把合成结果称为真人零漏检。地图、碰撞、绳子、剧情、道具和着地规则没有为迁就夹具而放宽。

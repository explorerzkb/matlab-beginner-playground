...
assert(size(data.player(1).high,1)==7 && isempty(data.player(1).low));
assert(size(data.player(1).segments,1)==7);
assert(size(data.player(2).high,1)==6 && size(data.player(2).low,1)==1);
assert(size(data.player(2).segments,1)==6);
expected=[240*(1-500/640) 180*80/480];
assert(min(vecnorm(data.player(1).high-expected,2,2))<1e-12, ...
    'Raw-camera point did not align with mirrored preview');
assert(all(data.player(1).high(:,1)<120) && all(data.player(2).high(:,1)>120), ...

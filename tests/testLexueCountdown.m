...
state.players(1).pos = [platform(1) + platform(3) / 2, ...
...
assert(size(state.levelState.oneWayPlatforms, 1) == ...
    size(lexue.countdownPlatforms, 1) + 2, ...
    'Lexue controls were not added as one-way physical platforms.');
end

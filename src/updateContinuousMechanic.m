...
[viewWidth,~]=cameraViewport(state,cfg);
halfView = viewWidth / 2 + 1.5;
...
active = struct( ...
    ...
    'bus', rangesOverlap(viewRange, [world.mechanic.bus.loopStart-2, world.worldWidth]));
...
visible = visible && ~bicycle.landed && ...
    ~(isfield(bicycle,'disappeared') && bicycle.disappeared);
if ~visible
...
function updateBuses(handles, state, world, ~, viewRange)
if ~rangesOverlap(viewRange, [world.mechanic.bus.loopStart-2, world.worldWidth])

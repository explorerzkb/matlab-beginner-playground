...
if ~isfield(state.levelState.network, 'rechargeCooldowns')
    ...
    state.levelState.network.rechargeFlash = zeros(1, 2);
end
state.levelState.network.rechargeCooldowns = max(0, ...
    state.levelState.network.rechargeCooldowns - dt);
state.levelState.network.rechargeFlash = max(0, ...
    state.levelState.network.rechargeFlash - dt);
for padIndex = 1:size(network.rechargePads, 1)
    for playerIndex = 1:2
        if state.levelState.network.rechargeCooldowns(playerIndex, ...
                padIndex) == 0 && playerOverlaps( ...
                state.players(playerIndex), ...
                network.rechargePads(padIndex, :))
            impulse = network.rechargeImpulses(padIndex, :);
            state.players(playerIndex).vel(1) = min(max( ...
                state.players(playerIndex).vel(1) + impulse(1), ...
                -network.rechargeMaxHorizontal), ...
                network.rechargeMaxHorizontal);
            state.players(playerIndex).vel(2) = min(max( ...
                state.players(playerIndex).vel(2), impulse(2)), ...
                network.rechargeMaxVertical);
            state.levelState.network.rechargeCooldowns(playerIndex, ...
                padIndex) = network.rechargeCooldown;
            state.levelState.network.rechargeFlash(padIndex) = 0.2;
        end
    end
end

...
try
    ...
    if duration>0
        ...
        report.perRoiPoseHz=report.captureCallHz*report.validUpdates/max(1,report.packets);
        report.perPlayerPoseHz=report.captureCallHz*report.boundUpdates/max(1,report.packets);
    ...
    report.notes=['Capture-call rate is not hardware exposure FPS. Per-player rates ', ...
        'are valid bound-player updates; separate ROI rates precede identity checks. ', ...
        'Neither rate proves identity accuracy. ', ...
    ...
    fprintf('POSE TELEMETRY: %s | capture %.2f Hz | ROI poses %.2f / %.2f Hz\n', ...
        folder,report.captureCallHz,report.perRoiPoseHz);
catch exception

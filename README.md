...
统一入口默认先启动双人体感和镜像校准；按 `K` 可随时切到键盘。摄像头、模型、支持包或后台进程在 Windows／macOS 上不可用、超时或报错时，程序会释放体感资源并自动进入键盘模式，不需要换启动文件。若想一开始就只用键盘，可运行 `startGame('InputMode','keyboard')`。
...
assets/game/             图片、MoveNet 模型和平台运行库等完整运行资源
